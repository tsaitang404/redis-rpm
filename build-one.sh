...
# --- dependencies (match official Dockerfile.rockylinux8/9) -------------------
dnf install -y -q gcc gcc-c++ make cmake wget tar gzip rpm-build \
  python3 python3-pip openssl-devel which 2>&1 | tail -1

# el8: use gcc-toolset-13 if available (matches official build)
...
# --- build command (8.x 'make build core', older plain make) -----------------
if [ "$MAJOR" -ge 8 ]; then
...
BuildRequires:  gcc, make, cmake, python3, openssl-devel
...
# SELinux policy (matches official)
...
# Allow redis to read sentinel config
...
# Fill in version/build placeholders
